...
explore_graphD
# hard to see trend, maybe linear?
...
#p-value: 0.0007672
# might want to change this one for a better fit?
...
model_plot <- ggplot(data, aes(x=Abatement))+
  geom_point(aes(y = MC_A, color = "MC_A"))+
  geom_smooth(aes(y = MC_A), method = lm, se = FALSE, size = 0.5, color = "gray20")+
  geom_point(aes(y = MC_B, color = "MC_B"))+
  geom_smooth(aes(y = MC_B), method = lm, se = FALSE, size = 0.5, color = "gray20")+
  geom_point(aes(y = MC_C, color = "MC_C"))+
  geom_smooth(aes(y = MC_C), method = lm, se = FALSE, size = 0.5, color = "gray20")+
  geom_point(aes(y = MC_D, color = "MC_D"))+
  geom_smooth(aes(y = MC_D), method = lm, se = FALSE, size = 0.5, color = "gray20")+
...
curve(d_A, from=0, to=300, xlab = "quantity", ylab = "price",  ylim = c(0,200), xlim = c(0,350), col = "yellow")
...
curve(d_C, from=0, to=300, xlab= "quantity", ylab = "price", add= TRUE,  ylim = c(0,200), xlim = c(0,350), col = "blue")
...
curve(d_D, from=0, to=400, xlab= "quantity", ylab = "price", add= TRUE, ylim = c(0,200), xlim = c(0,350), col = "red")
#WTP for first unit is $87.67


#sector willing to pay the most for the first unit of carbon emissions is C

#also should probably up the aesthetics on this 
